...
class ShopAdapter : RecyclerView.Adapter<ShopHolder>() {
    ...
    private val shops: MutableList<Purchase> = mutableListOf()
    ...
    fun setShops(products: List<Purchase>) {
        this.shops.clear()
...
class ShopHolder(view: View) : RecyclerView.ViewHolder(view) {
    ...
    fun bind(purchases: Purchase) {
        itemView.nameTextView.text = purchases.name
        itemView.sumTextView.text = "Cost: ${purchases.cost}"
